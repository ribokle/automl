"""Robust / quantile plugins recover elasticity sign + feed the predictor."""
...
@pytest.mark.parametrize("key", ["huber", "ransac", "theil_sen", "quantile"])
def test_recovers_negative_sign(key: str) -> None:
    result = registry.get(key).fit(_demand_frame(), FitContext(ppg_id="P1", controls=["ctrl"]))
    assert result.sign_ok, f"{key} got {result.own_elasticity}"
    assert "const" in result.coefficients


@pytest.mark.parametrize("key", ["huber", "ransac", "theil_sen", "quantile"])
def test_winner_drives_predictor(key: str) -> None:
    frame = _demand_frame()
