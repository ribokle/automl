"""LSTM sequence-forecaster plugin (optional dependency: torch). Forecast-only."""
...
@register
class LSTMPlugin(BaseModelPlugin):
    key = "lstm"
    family = "deep"
    ...
    def fit(self, frame: pd.DataFrame, ctx: FitContext) -> ModelResult:
        ...
        return fit_torch_seq(
            ...
            model_name=self.key, cell="lstm", test=ctx.test, hparams=hp,
        )
