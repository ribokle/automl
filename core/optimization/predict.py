...
def predict_units(
    ...
    *,
    ...
    # model (loglog / ridge / lasso / elasticnet / huber / ransac / …) is
    # log-price space.
...
def _build_cell_row(
    ...
    is_semilog = predictor.model_kind == "semilog_ols"
    swept_price_cols: set[str] = (
        {"price"} if is_semilog else {"log_price", "log_price_gap", "log_base_price"}
    )
